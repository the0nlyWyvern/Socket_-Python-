...
def update_database():
    while True:
        ...
        with open('database/Ty_gia_vang.json', mode='w') as f:
            get_http = json.loads(get(url_links[1]).text.encode().decode('utf-8-sig'))
...
def execution_a_client(cont, addr):
    ...
    send_msg(cont, GUI.combine(GUI.welcome(SERVER_NAME=socket.gethostname(), SERVER_IP=SERVER), GUI.help()))
            ...
            if msg == '-quit':
                ...
                if msg:
                    if verify_account(msg[0], msg[1]):
                        username = msg[0]
                        print(f'~{addr}~ Successfully access: {addr[0]} is "{username}"')
                        send_msg(cont,GUI.combine(f'Log in successfully! WELCOME "{username}\n"',GUI.look_up_instruction()))
                        MODE = 'look_up'
                    else:
                        print(f'~{addr}~ Access failure')
                        send_msg(cont, 'Username or Password may be wrong, type again:')
                else:
                ...
                if msg:
                    if register(msg[0], msg[1]):
                        username = msg[0]
                        print(f'~{addr}~ Account created with name: "{username}"')
                        send_msg(cont, GUI.combine(f'A new account has been created. WELCOME "{username}"\n', 
                                                   GUI.look_up_instruction()))
                        MODE = 'look_up'
                    else:
                        print(f'~{addr}~ Account is already exists')
                        send_msg(cont,'Account is already exists, you can type "-signin" to continue\nOr type again: ')
                else:
                    send_msg(cont, GUI.typo_error())
                ...
                send_msg(cont, 'I do not understand (may be you have not logged in)')
...
def run_server():
    ...
    while True:
        ...
        t2 = threading.Thread(target=execution_a_client, args=(connect, address))
        t2.start()
